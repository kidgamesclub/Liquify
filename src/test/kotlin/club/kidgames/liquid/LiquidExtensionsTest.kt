...
class LiquidExtensionsTest {
  ...
  @Test
  fun testFilters() {
    val rendered = renderer.render("Hello {{ player.displayName | red | bold }} and then some", player = player)
  ...
  fun testFormatTags() {
    ...
    val rendered = renderer.render("{%red%}{%bold%}{%yellow%}Am I Wry{%endyellow%}? {%green%}{%underline%}No{%endunderline%}{%endgreen%} by {%endbold%}Mew{%endred%} -- Eric", player = player)
    println(rendered.replace('§', '&'))
